...
3. 可观测性：`/api/task/{job_id}` 与 `/api/tasks` 暴露 `error_code` 与 `phase`，`/api/metrics` 暴露核心计数指标。
...
curl -sS -X POST http://127.0.0.1:18001/api/task \
  ...
  -d '{"jobs":[{"code_url":"https://github.com/madler/zlib.git","docker":true,"docker_image":"auto","total_time_budget":900,"run_time_budget":900,"max_tokens":1000}]}'
```
2. 提交失败任务（不存在仓库）：
```bash
curl -sS -X POST http://127.0.0.1:18001/api/task \
  -H 'Content-Type: application/json' \
  -d '{"jobs":[{"code_url":"https://github.com/example/not-exist-repo.git","docker":true,"docker_image":"auto","total_time_budget":300,"run_time_budget":120,"max_tokens":200}]}'
```
3. 轮询详情与列表：
```bash
...
3. 字段可观测性：
   - 详情：`job_id`、`phase`、`error_code`、`children_status`
   - 列表：`job_id`、`phase`、`error_code`、`active_child_*`
4. 指标可观测性：
   - `sherpa_jobs_total`

...
### 2.2 状态跟踪与结果回收
- [x] web 能轮询/监听 Job 状态（Pending/Running/Succeeded/Failed）
- [x] Job 结束后 web 能回收输出路径与摘要
- [x] 超时、失败状态可落库并可查询

### 2.3 幂等保护
- [x] 同 `job_id` 创建前先做存在性检查
- [x] 避免重复创建多个 Job（唯一键或 CAS/锁）
- [x] 对重复触发返回可解释结果（已在执行/已完成）

**验收证据**
- [x] 多阶段 Job（plan/synthesize/build/run）从提交到产物可闭环
- [x] 重复触发不产生重复执行

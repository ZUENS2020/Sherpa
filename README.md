...
10. 任务执行器支持 `local_thread`（默认）与 `k8s_job`（去 DinD）两种模式，由 `SHERPA_EXECUTOR_MODE` 控制。
...
### 2.2 Kubernetes 迁移资产（SHE-52）

1. 基础清单：`k8s/base/`（`kubectl apply -k k8s/base`）
2. 映射文档：`docs/k8s/MAPPING.md`
...
6. `sherpa-web` 通过 `DATABASE_URL` 连接 Postgres 持久化任务状态。
7. Postgres 作为状态存储唯一后端，缺失 `DATABASE_URL` 时 web 启动会失败。
...
| `postgres` | 任务状态数据库（Postgres） | 内部 `:5432` |
...
# 6) 查看 SQLite 任务状态库
docker compose exec postgres psql -U ${POSTGRES_USER:-sherpa} -d ${POSTGRES_DB:-sherpa} -c '\\dt'
docker compose exec postgres psql -U ${POSTGRES_USER:-sherpa} -d ${POSTGRES_DB:-sherpa} -c 'select job_id,status,updated_at from jobs order by updated_at desc limit 20;'
```
...
| `DATABASE_URL` | `postgresql://sherpa:sherpa@postgres:5432/sherpa` | Postgres 连接串（必填） |

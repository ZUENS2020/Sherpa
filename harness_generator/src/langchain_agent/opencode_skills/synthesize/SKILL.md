...
- Must include:
  ...
  - primary runnable fuzzers must link the libFuzzer runtime with `-fsanitize=fuzzer,address,undefined -fsanitize-coverage=trace-pc-guard,inline-8bit-counters` (or equivalent sanitizer set including `fuzzer` with coverage feedback); do not use `-fsanitize=fuzzer-no-link` for final binaries unless you also provide and link a valid fuzzer `main`

...
def test_web_dockerfile_node_download_has_timeout_and_mirror_fallback() -> None:
    ...
    candidates = re.search(r'ARG NODE_MIRROR_CANDIDATES="([^"]+)"', text)

    assert candidates is not None
    mirrors = candidates.group(1).split()
    assert mirrors[0] == "https://nodejs.org/dist"
    assert "https://npmmirror.com/mirrors/node" in mirrors
    assert 'for base in $NODE_MIRROR_CANDIDATES' in text
    assert "node_download_ok" in text
    assert "--retry-max-time 180" in text
    assert "--connect-timeout 10" in text
    assert "--max-time 90" in text
    assert "--speed-time 20" in text
    assert "--max-time 300" not in text
